...
@Component({
  ...
  styleUrls: ['./registro-persona-juridica.component.css']
})
export class RegistroPersonaJuridicaComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.tipoPersona = AppConst.TIPO_PERSONA_JURIDICA;
  ...
  openModelCrearUsuario(crearUsuario:TemplateRef<any>){
    this.modalRefCrearUsuario = this.modalService.show(crearUsuario);
  }
  ...
  habilitarDeshabilitarBtoCrear(){
    if(this.personaInfo.apellido != '' 
      ...
      && this.personaInfo.razonSocial != ''
      && this.personaInfo.nit != ''
